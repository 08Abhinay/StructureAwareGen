...
def calc_metric_pr(metric, **kwargs): # See metric_utils.MetricOptions for the full list of arguments.
    assert is_valid_metric(metric)
    opts = metric_utils_pr.MetricOptions(**kwargs)
...
@register_metric
def pr50k3_full_cond(opts):
    # make sure you don’t accidentally cap your dataset
    opts.dataset_kwargs.update(max_size=None, xflip=False)

    # run the class‐conditional PR
    cls_P, cls_R, macro_P, macro_R = pr_class_cond.compute_pr_class_cond(
        opts,
        max_real       = 200_000,
        num_gen        =  50000,
        nhood_size     =      3,
        row_batch_size =  10_000,
        col_batch_size =  10_000,
    )

    # build the metric dict
    results = {
        'pr50k3_full_macro_precision': macro_P,
        'pr50k3_full_macro_recall':    macro_R,
    }

    # and add each class individually
    for cls in sorted(cls_P):
        results[f'pr50k3_full_precision_class_{cls}'] = cls_P[cls]
        results[f'pr50k3_full_recall_class_{cls}']    = cls_R[cls]
