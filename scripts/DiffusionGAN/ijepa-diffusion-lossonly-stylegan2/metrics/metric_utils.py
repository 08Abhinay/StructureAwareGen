...
#             flag = bool(flag.cpu().item())

#         if flag:
#             stats = FeatureStats.load(cache_file)
#             if capture_labels and hasattr(stats, 'labels'):
#                 return stats
#             elif not capture_labels:
#                 return stats
...
#     # prepare label collection if requested
#     if capture_labels and opts.rank == 0:
#         all_labels = []
...
#         # images: (B, C, H, W), labels: whatever your dataset returns
#         if images.shape[1] == 1:
#             images = images.repeat([1, 3, 1, 1])
...
#     # attach labels tensor if requested
#     if capture_labels and opts.rank == 0:
#         stats.labels = torch.from_numpy(np.concatenate(all_labels, axis=0)).to(opts.device)
