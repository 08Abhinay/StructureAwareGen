...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, e_ijepa, sync):
        # Mapping (always returns ws, film)
        with misc.ddp_sync(self.G_mapping, sync):
            ws, film = self.G_mapping(z, c, e_ijepa=e_ijepa)
            ...
            if self.style_mixing_prob > 0:
                ...
                mix_ws, _ = self.G_mapping(
                    torch.randn_like(z), c, e_ijepa=e_ijepa, skip_w_avg_update=True)
                ws[:, cutoff:] = mix_ws[:, cutoff:]

        # Synthesis
        with misc.ddp_sync(self.G_synthesis, sync):
            img = self.G_synthesis(ws, film=film)
        return img, ws

    def run_D(self, img, c, e_ijepa, sync):
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                gen_logits = self.run_D(
                    gen_img, gen_c,
                    e_ijepa=scaled_target_f,         # same embedding 
                    sync=False
                )
        ...
        if do_Gpl:
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                gen_img, gen_ws = self.run_G(gen_z[:batch_size], gen_c[:batch_size], e_ijepa=zero_embed[:batch_size], sync=sync)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, e_ijepa=zero_embed, sync=False)
                e_fake = self._feat(gen_img).detach() 
                gen_logits = self.run_D(gen_img, gen_c, e_ijepa=zero_embed, sync=False)  # Gets synced by loss_Dreal.
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                e_real = self._feat(real_img_tmp).detach()
                real_logits = self.run_D(real_img_tmp, real_c, e_ijepa=zero_embed, sync=sync)
