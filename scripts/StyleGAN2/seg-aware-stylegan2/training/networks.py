...
@persistence.persistent_class
class SegCrossAttention2d(torch.nn.Module):
    def __init__(self, in_channels, token_dim=256, d_model=256, n_heads=8):
        super().__init__()
        self.in_channels = in_channels
        self.token_dim = token_dim
        self.d_model = d_model
        self.q_proj = torch.nn.Linear(in_channels, d_model)
        self.token_proj = torch.nn.Linear(token_dim, d_model)
        self.q_ln = torch.nn.LayerNorm(d_model)
        self.token_ln = torch.nn.LayerNorm(d_model)
        self.attn = torch.nn.MultiheadAttention(embed_dim=d_model, num_heads=n_heads, batch_first=True)
        self.out_proj = torch.nn.Linear(d_model, in_channels)
        self.gate = torch.nn.Parameter(torch.zeros([]))

    def forward(self, x, tokens, pad_mask, cond_ramp=1.0):
        B, C, H, W = x.shape
        x_float = x.to(torch.float32)
        tokens_float = tokens.to(torch.float32)
        if pad_mask is not None:
            pad_mask = pad_mask.to(device=tokens.device)

        q = x_float.flatten(2).permute(0, 2, 1)
        q = self.q_ln(self.q_proj(q))
        kv = self.token_ln(self.token_proj(tokens_float))
        attn_out, _ = self.attn(q, kv, kv, key_padding_mask=pad_mask, need_weights=False)
        attn_out = self.out_proj(attn_out).permute(0, 2, 1).reshape(B, C, H, W)

        gate = self.gate.to(x.dtype)
        return x + gate * float(cond_ramp) * attn_out.to(x.dtype)

@persistence.persistent_class
class SegTokenPool(torch.nn.Module):
    def __init__(self, token_dim=256, d_model=256, n_heads=8):
        super().__init__()
        self.token_dim = token_dim
        self.d_model = d_model
        self.token_proj = torch.nn.Linear(token_dim, d_model)
        self.token_ln = torch.nn.LayerNorm(d_model)
        self.query = torch.nn.Parameter(torch.zeros([1, 1, d_model]))
        self.query_ln = torch.nn.LayerNorm(d_model)
        self.attn = torch.nn.MultiheadAttention(embed_dim=d_model, num_heads=n_heads, batch_first=True)
    ...
    def forward(self, tokens, pad_mask):
        ...
        q = self.query_ln(q)
        pooled, _ = self.attn(q, kv, kv, key_padding_mask=pad_mask, need_weights=False)
...
class IJEPAFusionMapping(torch.nn.Module):
    ...
    def __init__(
        ...
        **map_kwargs,               # Forwarded to MappingNetwork.
        ...
        self.alpha             = torch.nn.Parameter(torch.tensor(fusion_alpha))  # learnable
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        self.num_conv += 1
        self.seg_attn = SegCrossAttention2d(out_channels) if resolution in [16, 32] else None
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None,
                seg_tokens=None, seg_pad_mask=None, seg_ramp=1.0, **layer_kwargs):
        ...
        if self.seg_attn is not None and seg_tokens is not None:
            x = self.seg_attn(x, seg_tokens, seg_pad_mask, cond_ramp=seg_ramp)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, seg_tokens=None, seg_pad_mask=None, seg_ramp=1.0, **block_kwargs):
        block_ws = []
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            ...
            x, img = block(x, img, cur_ws,
                           seg_tokens=seg_tokens, seg_pad_mask=seg_pad_mask, seg_ramp=seg_ramp,
                           **block_kwargs)
        return img
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, e_ijepa=None, truncation_psi=1, truncation_cutoff=None,
                sem_ramp=1.0, seg_tokens=None, seg_pad_mask=None, seg_ramp=1.0, **synthesis_kwargs):
        ws = self.mapping(
            z, c,
            e_ijepa=e_ijepa,
            sem_ramp=sem_ramp,
        ...
        img = self.synthesis(ws, seg_tokens=seg_tokens, seg_pad_mask=seg_pad_mask, seg_ramp=seg_ramp, **synthesis_kwargs)
        return img
...
class DiscriminatorEpilogue(torch.nn.Module):
    ...
    def forward(self, x, img, cmap, force_fp32=False):
        ...
        if self.cmap_dim > 0:
            if cmap is None:
                x = x.sum(dim=1, keepdim=True) * (1 / np.sqrt(self.cmap_dim))
            else:
...
class Discriminator(torch.nn.Module):
    def __init__(self,
        ...
        if cmap_dim > 0:
            ...
            self.seg_pool = SegTokenPool(token_dim=256, d_model=256, n_heads=8)
            self.proj_seg_d = torch.nn.Sequential(
                FullyConnectedLayer(256, 512, activation='lrelu', lr_multiplier=1.0),
                FullyConnectedLayer(512, cmap_dim, activation='linear', lr_multiplier=1.0),
            )
        else:
            self.proj_ijepa_d = None
            self.seg_pool = None
            self.proj_seg_d = None
    ...
    def forward(self, img, c, e_ijepa=None, seg_tokens=None, seg_pad_mask=None,
                sem_ramp=0.0, seg_ramp=0.0, **block_kwargs):
        x = None
        for res in self.block_resolutions:
            block = getattr(self, f'b{res}')
        ...
        B      = x.shape[0]
        device = x.device
        if self.b4.cmap_dim == 0:
            return self.b4(x, img, None)
        # cmap = None
        # if self.c_dim > 0:
        ...
        sem_weight = max(min(float(sem_ramp), 1.0), 0.0)
        seg_weight = max(min(float(seg_ramp), 1.0), 0.0)

        # ----- Compute projected IJEPA embedding -----
        cmap_e = torch.zeros(B, self.b4.cmap_dim, device=device)
        if (e_ijepa is not None) and (self.proj_ijepa_d is not None):
            e_eff = sem_weight * e_ijepa
        ...
        # ----- Compute segmentation embedding if available -----
        cmap_seg = torch.zeros_like(cmap_e)
        if (seg_tokens is not None) and (self.proj_seg_d is not None):
            pooled = self.seg_pool(seg_tokens, seg_pad_mask)
            cmap_seg = normalize_2nd_moment(self.proj_seg_d(seg_weight * pooled))

        # ----- Blend with ramp-based weighting -----
        w_c = max(1.0 - sem_weight - seg_weight, 0.0)
        w_e = sem_weight
        w_s = seg_weight
        if not ((self.c_dim > 0) and (self.mapping is not None)):
            w_c = 0.0
        if not ((e_ijepa is not None) and (self.proj_ijepa_d is not None)):
            w_e = 0.0
        if not ((seg_tokens is not None) and (self.proj_seg_d is not None)):
            w_s = 0.0
        ...
        if w_sum > 0.0:
            ...
            w_e /= w_sum
            w_s /= w_sum
            cmap = normalize_2nd_moment(w_c * cmap_c + w_e * cmap_e + w_s * cmap_seg)
        else:
            cmap = None
