...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    imgs = [p for p in sorted(Path(source_dir).rglob("*"))
            if is_image_ext(p) and p.is_file()]

    # 2) Build class→ID mapping from the first‐level subfolders
    class_names = sorted({p.parent.name for p in imgs})
...
#         # Fix images with alpha channel or other anomalies by forcing RGB.
#         if img.ndim == 3 and img.shape[2] > 3:
#             img = img[:, :, :3]  # Strip alpha or extra channels
...
def convert_dataset(
    ...
    dataset_attrs = None
    labels = []
    channel_counter = {1: 0, 3: 0}

    # First pass to find the dominant channel format (1 or 3)
    temp_images = []
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        img = transform_image(image['img'])
        if img is None:
            continue
        channels = img.shape[2] if img.ndim == 3 else 1
        if channels in channel_counter:
            channel_counter[channels] += 1
        temp_images.append((idx, image, img))

    dominant_channels = 3 if channel_counter[3] >= channel_counter[1] else 1

    for idx, image, img in temp_images:
        idx_str = f'{idx:08d}'
        archive_fname = f'{idx_str[:5]}/img{idx_str}.png'

        # Fix images with alpha channel or extra channels
        if img.ndim == 3 and img.shape[2] > 3:
            img = img[:, :, :3]  # Strip alpha or extras

        # Convert to dominant channel format
        if dominant_channels == 3:
            if img.ndim == 2:
                img = np.stack([img] * 3, axis=-1)  # Gray to RGB
            elif img.shape[2] == 1:
                img = np.repeat(img, 3, axis=2)
        elif dominant_channels == 1:
            if img.ndim == 3 and img.shape[2] == 3:
                img = np.mean(img, axis=2).astype(np.uint8)  # RGB to grayscale
