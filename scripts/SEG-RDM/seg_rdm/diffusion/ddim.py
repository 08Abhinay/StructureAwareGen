...
from seg_rdm.diffusion.schedules import make_ddim_sampling_parameters, make_ddim_timesteps
from seg_rdm.diffusion.utils import noise_like
...
class DDIMSampler(object):
    ...
    def ddim_sampling(
        ...
        input_scale = getattr(self.model, "input_scale", 1.0)
        return img / input_scale, intermediates
    ...
    def p_sample_ddim(
        ...
        broadcast_shape = (b,) + (1,) * (x.ndim - 1)
        a_t = torch.full(broadcast_shape, alphas[index], device=device)
        a_prev = torch.full(broadcast_shape, alphas_prev[index], device=device)
        sigma_t = torch.full(broadcast_shape, sigmas[index], device=device)
        sqrt_one_minus_at = torch.full(broadcast_shape, sqrt_one_minus_alphas[index], device=device)
        ...
        if quantize_denoised:
            if hasattr(self.model, "first_stage_model") and hasattr(self.model.first_stage_model, "quantize"):
                ...
                raise ValueError("quantize_denoised=True requires a first_stage_model with quantize().")
