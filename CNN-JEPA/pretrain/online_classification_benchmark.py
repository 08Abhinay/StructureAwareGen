...
import inspect, torch.utils.data
print(">>> DEBUG: torch.utils.data.DataLoader ↦", inspect.getfile(torch.utils.data.DataLoader))
...
class OnlineLinearClassificationBenckmark:
    ...
    def run_benchmarks(self,
        ...
        print(">>> DEBUG – run_benchmarks received:")
        print("    type(train_dataloader) =", type(train_dataloader))
        print("    type(val_dataloader)   =", type(val_dataloader))
        assert train_dataloader.dataset.transform == train_val_transform, "train_dataloader.dataset.transform != train_val_transform, adapt this and the lines where we change the transform to the dataset"
        train_dataloader.dataset.transform = self.transform

        # val_dataloader.dataset is expected to be a LightlyDataset
        assert val_dataloader.dataset.transform == train_val_transform, "val_dataloader.dataset.transform != train_val_transform, adapt this and the lines where we change the transform to the dataset"
        ...
        train_dataloader.dataset.transform = train_val_transform
    ...
    def evaluate_lin_classifier(self, val_features, val_labels, classifier):
        ...
        max_k = min(max(self.topk), self.num_classes)
        _, predicted_classes = outputs.topk(max_k, dim=1, largest=True, sorted=True)
